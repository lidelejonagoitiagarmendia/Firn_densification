...
def solve_forward_problem(H,Hres,acc_rate,K,n,Aglen,rho_snow,rho_ice=910,phi_snow=0.4,ab_phi_lim=0.81):
...
sites_names=['DYE3','GRIP','NEEM','NGRIP','SITE2','SITEA']
...
Aglens=np.load('Aglens_inv_raw.npy')
...
for i in range(len(sites)):
    ...
    rho_raw, z_raw=get_rho_data(H,site)
    z_raw_smooth,rho_raw_smooth,rho_surf=smooth_and_extrapolate_raw_data(z_raw,rho_raw,H)
    acc_rate=acc_from_iceeqyr_to_snoweqs(acc_sites[site],rho_surf)

    rho_k10,z_rho_k10=solve_forward_problem(H,Hres,acc_rate,Ks[0],n,Aglens[i,0],rho_surf)      
    rho_k100,z_rho_k100=solve_forward_problem(H,Hres,acc_rate,Ks[1],n,Aglens[i,1],rho_surf)      
    rho_k1000,z_rho_k1000=solve_forward_problem(H,Hres,acc_rate,Ks[2],n,Aglens[i,2],rho_surf)
    rho_k10000,z_rho_k10000=solve_forward_problem(H,Hres,acc_rate,Ks[3],n,Aglens[i,3],rho_surf)
    

    fig,ax=plt.subplots(figsize=(6,6))

    ax.scatter(rho_raw/rho_ice, -abs(z_raw-max(z_raw)), s=30,color='grey',label=name+' measured data',facecolor='white',edgecolor='grey',alpha=0.8)
    ax.plot(rho_k10/rho_ice,-abs(z_rho_k10-max(z_rho_k10)),color='mediumblue',label=r'$K=10^1$, $\bar{A}$='+str(np.format_float_scientific(Aglens[i,0], precision=2,exp_digits=2))+' s$^{-1}$Pa$^{-3}$')
    ax.plot(rho_k100/rho_ice,-abs(z_rho_k100-max(z_rho_k100)),color='green',label=r'$K=10^2$, $\bar{A}$='+str(np.format_float_scientific(Aglens[i,1], precision=2,exp_digits=2))+' s$^{-1}$Pa$^{-3}$')
    ax.plot(rho_k1000/rho_ice,-abs(z_rho_k1000-max(z_rho_k1000)),color='red',label=r'$K=10^3$, $\bar{A}$='+str(str(np.format_float_scientific(Aglens[i,2], precision=2,exp_digits=2)))+' s$^{-1}$Pa$^{-3}$')
    ax.plot(rho_k10000/rho_ice,-abs(z_rho_k10000-max(z_rho_k10000)),color='orange',label=r'$K=10^4$, $\bar{A}$='+str(str(np.format_float_scientific(Aglens[i,3], precision=2,exp_digits=2)))+' s$^{-1}$Pa$^{-3}$')
    ...
    fig.savefig('density_profiles_'+site+'.png',dpi=300,format='png')
    fig.show()
