...
def solve_forward_problem(H,Hres,acc_rate,K,n,Aglen,rho_obsfit,Tsite,Flux,rho_ice=910,phi_snow=0.4,ab_phi_lim=0.81):
    ...
    Qele = FiniteElement("CG", mesh.ufl_cell(), 1) #Temperature
    U, V, Q = FunctionSpace(mesh, Uele), FunctionSpace(mesh, Vele), FunctionSpace(mesh, Qele) 
    MixedEle = MixedElement([Uele, Vele, Qele]) 
    W = FunctionSpace(mesh, MixedEle)

    (wr,wv, wt) = TestFunctions(W) #weight functions (Galerkin method)
    w = Function(W) #container for solution (rho,v)
    (rho,v, T) = split(w)

    #Get nodal coordinates
    z_rho = U.tabulate_dof_coordinates()[:,0] #density node coordinates
    z_v  = V.tabulate_dof_coordinates()[:,0] #velocity node coordinates
    z_T = Q.tabulate_dof_coordinates()[:,0] #temperature node coordinates
    I_rho, I_v, I_T = np.argsort(z_rho), np.argsort(z_v), np.argsort(z_T)
    z_rho, z_v, z_T = z_rho[I_rho], z_v[I_v], z_T[I_T] # sorted list of coordinates
    ...
    bc_T_s=DirichletBC(W.sub(2),T_sfc,boundary_subdomains,2) #Temperature at the surface

    #-----------------------------------BOTTOM
    bc_v_b=DirichletBC(W.sub(1),v_bot,boundary_subdomains,1) #Velocity at the bottom

    bcs=[bc_rho_s,bc_v_s,bc_T_s,bc_v_b]      

    #----------------------------------------INITIAL STATE--------------------------------------------#

    assigner = FunctionAssigner(W, [U, V, Q]) # used to assign values into components of mixed function space
    ...
    tenp_init = Expression("T_surf-(T_surf-T_bot)*pow((H-x[0])/H,0.1)", H=H, T_bot=T_bed, T_surf=T_sfc, degree=2)

    rho_init, v_init, t_init = interpolate(r_init, U), interpolate(u_init, V), interpolate(tenp_init,Q)
    w_init = Function(W) # container for solution (uz,rho,T)
    assigner.assign(w_init, [rho_init,v_init,t_init]) # assign initial guesses
    w.assign(w_init)
    ...
    c=c0+c1*(T-T0)
    k=(kf0-kf1*rho+kf2*rho**2)/(kf0-kf1*rho_ice+kf2*rho_ice**2) * k0*exp(-gammaT*T)
    
    a_T = rho*c*v*T.dx(0)*wt*dx + k*T.dx(0)*wt.dx(0)*dx - ssol*wt*dx - k.dx(0)*T.dx(0)*wt*dx
    L_T = 0.0
...
for site in sites:
    ...
    for K in Ks:
        ...
        rho,z=solve_forward_problem(H,Hres,acc_rate,K,n,Aglen,rho_obsfit,Tsite,Flux,rho_ice=910,phi_snow=0.4,ab_phi_lim=0.81)

        np.save('./SSresults/rho_1D_'+str(site)+'_withT_K='+str(K)+'.npy',rho)
        np.save('./SSresults/z_1D_'+str(site)+'_withT_K='+str(K)+'.npy',z)
